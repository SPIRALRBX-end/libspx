...
// Predição linear (mesma do encoder)
...
int read_samples(audio_decoder_t *d, int16_t *buf, int max_samps) {
    ...
    case FORMAT_SPX: {
        ...
        int decompressed_size = decompress_rle(spx_data->compressed_buffer, compressed_size, temp_buf);
        ...
        for (int i = 0; i < decompressed_size && samples_decoded < max_samps; i++) {
            ...
            for (int j = 0; j < 2 && samples_decoded < max_samps; j++) {
                ...
                buf[samples_decoded] = (int16_t)reconstructed;
                
                // Atualiza histórico
                memmove(&spx_data->history[c * 3], &spx_data->history[c * 3 + 1], 2 * sizeof(int16_t));
                spx_data->history[c * 3 + 2] = (int16_t)reconstructed;
